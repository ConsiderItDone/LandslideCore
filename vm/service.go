...
type (
	...
	ABCIService interface {
		...
		ABCIQuery(ctx *rpctypes.Context, path string, data tmbytes.HexBytes, height int64, prove bool) (*ctypes.ResultABCIQuery, error)
...
func NewService(vm *VM) *LocalService {
	return &LocalService{vm}
}

func NewServiceAsRPCRoutes(vm *VM) map[string]*rpcserver.RPCFunc {
	s := NewService(vm)
	return map[string]*rpcserver.RPCFunc{
		// subscribe/unsubscribe are reserved for websocket events.
		"subscribe":       rpcserver.NewWSRPCFunc(s.Subscribe, "query"),
		"unsubscribe":     rpcserver.NewWSRPCFunc(s.Unsubscribe, "query"),
		"unsubscribe_all": rpcserver.NewWSRPCFunc(s.UnsubscribeAll, ""),

		// info API
		"health":               rpcserver.NewRPCFunc(s.Health, ""),
		"status":               rpcserver.NewRPCFunc(s.Status, ""),
		"net_info":             rpcserver.NewRPCFunc(s.NetInfo, ""),
		"blockchain":           rpcserver.NewRPCFunc(s.BlockchainInfo, "minHeight,maxHeight"),
		"genesis":              rpcserver.NewRPCFunc(s.Genesis, ""),
		"genesis_chunked":      rpcserver.NewRPCFunc(s.GenesisChunked, "chunk"),
		"block":                rpcserver.NewRPCFunc(s.Block, "height"),
		"block_by_hash":        rpcserver.NewRPCFunc(s.BlockByHash, "hash"),
		"block_results":        rpcserver.NewRPCFunc(s.BlockResults, "height"),
		"commit":               rpcserver.NewRPCFunc(s.Commit, "height"),
		"check_tx":             rpcserver.NewRPCFunc(s.CheckTx, "tx"),
		"tx":                   rpcserver.NewRPCFunc(s.Tx, "hash,prove"),
		"tx_search":            rpcserver.NewRPCFunc(s.TxSearch, "query,prove,page,per_page,order_by"),
		"block_search":         rpcserver.NewRPCFunc(s.BlockSearch, "query,page,per_page,order_by"),
		"validators":           rpcserver.NewRPCFunc(s.Validators, "height,page,per_page"),
		"dump_consensus_state": rpcserver.NewRPCFunc(s.DumpConsensusState, ""),
		"consensus_state":      rpcserver.NewRPCFunc(s.ConsensusState, ""),
		"consensus_params":     rpcserver.NewRPCFunc(s.ConsensusParams, "height"),
		"unconfirmed_txs":      rpcserver.NewRPCFunc(s.UnconfirmedTxs, "limit"),
		"num_unconfirmed_txs":  rpcserver.NewRPCFunc(s.NumUnconfirmedTxs, ""),

		// tx broadcast API
		"broadcast_tx_commit": rpcserver.NewRPCFunc(s.BroadcastTxCommit, "tx"),
		"broadcast_tx_sync":   rpcserver.NewRPCFunc(s.BroadcastTxSync, "tx"),
		"broadcast_tx_async":  rpcserver.NewRPCFunc(s.BroadcastTxAsync, "tx"),

		// abci API
		"abci_query": rpcserver.NewRPCFunc(s.ABCIQuery, "path,data,height,prove"),
		"abci_info":  rpcserver.NewRPCFunc(s.ABCIInfo, ""),
	}
}
...
func (s *LocalService) Subscribe(ctx *rpctypes.Context, query string) (*ctypes.ResultSubscribe, error) {
	...
	s.vm.log.Info("Subscribe to query", "remote", addr, "query", query)
		...
		for {
			...
			case msg := <-sub.Out():
				...
				if err = ctx.WSConn.WriteRPCResponse(writeCtx, resp); err != nil {
					s.vm.log.Info("Can't write response (slow client)",
...
func (s *LocalService) Unsubscribe(ctx *rpctypes.Context, query string) (*ctypes.ResultUnsubscribe, error) {
	addr := ctx.RemoteAddr()
	s.vm.log.Info("Unsubscribe from query", "remote", addr, "query", query)
	q, err := tmquery.New(query)
	if err != nil {
		return nil, fmt.Errorf("failed to parse query: %w", err)
	}
...
func (s *LocalService) UnsubscribeAll(ctx *rpctypes.Context) (*ctypes.ResultUnsubscribe, error) {
	addr := ctx.RemoteAddr()
	s.vm.log.Info("Unsubscribe from all", "remote", addr)
...
func (s *LocalService) TxSearch(ctx *rpctypes.Context, query string, prove bool, pagePtr, perPagePtr *int, orderBy string) (*ctypes.ResultTxSearch, error) {
...
func (s *LocalService) BlockchainInfo(ctx *rpctypes.Context, minHeight, maxHeight int64) (*ctypes.ResultBlockchainInfo, error) {
	...
	s.vm.log.Debug("BlockchainInfoHandler", "maxHeight", maxHeight, "minHeight", minHeight)
...
func (s *LocalService) CheckTx(ctx *rpctypes.Context, tx types.Tx) (*ctypes.ResultCheckTx, error) {
	res, err := s.vm.app.Mempool().CheckTxSync(abci.RequestCheckTx{Tx: tx})
	if err != nil {
		return nil, err
	}
